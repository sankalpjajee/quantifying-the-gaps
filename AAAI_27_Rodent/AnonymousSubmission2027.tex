...
Validated through a 2024 national big data health science case competition \citep{bdhsc2024case}, our system achieved a classification accuracy of 91.5\% with macro-averaged precision, recall, and F1-scores of 86.8\%, 81.2\%, and 83.5\%, outperforming the baselines we evaluated. % REQUIRES_EXPERIMENT: per-class metrics are not yet reported; the gap between accuracy and macro recall indicates the minority REM class is scored substantially below the majority states. By replacing subjective, labor-intensive scoring with scalable automation, this framework enables large-cohort analysis, supports reproducible science, and accelerates preclinical research on sleep mechanisms and therapies. Ultimately, this work contributes a significant step toward the standardization and democratization of sleep research protocols, supporting translational discoveries that benefit clinical understanding and treatment of sleep disorders.
...
Boosting algorithms such as XGBoost \citep{Chen_2016} exploit non-linear feature interactions and handle imbalanced data gracefully. \citet{saevskiy2025_sensors} reported 89\% accuracy using single-channel EEG in mice. Our ensemble extends this line of work with additional time-domain descriptors, including the MMD feature, reaching 91.5\% accuracy under the evaluation protocol described below. We note that higher accuracies have been reported by EMG-assisted deep models \citep{jha2024_slumbernet}; our contribution is not a new accuracy record but a compact, interpretable, calibrated feature basis for the single-channel EEG setting.

\subsection{Deep Learning Models}
Deep networks learn end-to-end representations directly from raw waveforms. The strongest mouse-specific example is SlumberNet, a residual network (ResNet) over paired EEG and EMG that reports 97\% accuracy and a 96\% F1-score across baseline, sleep-deprivation, and recovery conditions \citep{jha2024_slumbernet}. Though powerful, such networks demand paired EMG, substantial labeled data, and GPU resources not always available in preclinical labs.

\subsection{Lightweight and Single-Channel Solutions}
To lower cost and tethering burden, single-lead or EMG-free pipelines have emerged. SleepyRat provides a cloud interface for classical scoring plus simple neural nets \citep{sleepyrat2025_platform}. Downstream of scoring, SleepInvestigatoR standardizes the analysis of \emph{already-scored} rodent sleep files, computing 23 architecture measures from a single configurable R function \citep{gamble2025_sleepinvestigator}; it is complementary to, not competitive with, an automated scorer, and it illustrates how much of the field's tooling still presupposes that scoring has already happened. Such low-resource strategies motivate our emphasis on handcrafted time--frequency features that remain robust when training data are scarce.
...
Our model was evaluated on both the validation set (30\% of the training data) and the test set provided in the competition. Performance was measured using accuracy, precision, recall, and F1-score.
...
\end{axis}
\end{tikzpicture}
\caption{Comparison of the feed-forward neural network (NeuralNet) and XGBoost on sleep stage classification.}
\label{fig:barchart}
\end{figure}
...
The automated framework addresses critical limitations of manual scoring by eliminating inter-rater variability, reducing analysis time, and enabling large-scale phenotyping studies. The margin between overall accuracy and macro-averaged recall indicates that performance is not uniform across states, and per-class reporting is required before the method is relied upon for REM-specific analyses. % REQUIRES_EXPERIMENT: emit per-class precision/recall/F1, a confusion matrix, and Cohen's kappa.
Subject to that caveat, the approach is a promising tool for sleep-dependent research, circadian studies, and pharmacological investigations. By automating fundamental sleep analysis with high accuracy and reliability, this work removes significant barriers to large-scale sleep studies and advances our understanding of sleep mechanisms underlying health and disease.

\bibliography{references}

% Check whether the conference requires a reproducibility checklist to be included in the paper.
% AAAI-27 collects it as a separate file, so ReproducibilityChecklist.tex is compiled
% standalone (see ReproducibilityChecklist.pdf). If your track asks for it inline instead,
